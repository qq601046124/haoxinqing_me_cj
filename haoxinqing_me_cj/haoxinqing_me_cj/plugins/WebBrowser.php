 ...
 *
 */
class haoxinqing_me_cjObjectPlugin_WebBrowser {
	/**
	...
	public static function WebBrowser($self, $callback = null, $location = null) {
		...
		if (! $location)
			...
			$self->bind('click', array($location, $callback), array('haoxinqing_me_cjPlugin_WebBrowser', 'hadleClick'));
			$self->bind('submit', array($location, $callback), array('haoxinqing_me_cjPlugin_WebBrowser', 'handleSubmit'));
		}
	}
	...
	public static function downloadTo($self, $dir = null, $filename = null) {
		...
		if ($url) {
			...
			haoxinqing_me_cj::debug("Requesting download of $url\n");
	...
	public static function location($self, $url = null) {
		...
		$xhr = isset($self->document->xhr)
			? $self->document->xhr
			: null;
		$xhr = haoxinqing_me_cj::ajax(array(
			'url' => $url,
		), $xhr);
		$return = false;
		if ($xhr->getLastResponse()->isSuccessful()) {
			$return = haoxinqing_me_cjPlugin_WebBrowser::browserReceive($xhr);
...
class haoxinqing_me_cjPlugin_WebBrowser {
	/**
	 *
	 * @param $url
	 * @param $callback
	...
	public static function browserGet($url, $callback,
		...
		haoxinqing_me_cj::debug("[WebBrowser] GET: $url");
		...
		if ($xhr->getLastResponse()->isSuccessful()) {
			haoxinqing_me_cj::callbackRun($callback,
				array(self::browserReceive($xhr)->WebBrowser()),
				$paramStructure
			);
//			haoxinqing_me_cj::callbackRun($callback, array(
...
//			));
	...
	protected static function authorizeHost($url) {
		...
		if ($host)
			haoxinqing_me_cj::ajaxAllowHost($host);
	}
	...
	public static function browserReceive($xhr) {
		haoxinqing_me_cj::debug("[WebBrowser] Received from ".$xhr->getUri(true));
		...
		$pq = haoxinqing_me_cj::newDocument($body);
		$pq->document->xhr = $xhr;
...
//			print htmlspecialchars(var_export($xhr->getLastResponse()->getHeader('Set-Cookie'), true));
			haoxinqing_me_cj::debug("Meta redirect... '{$refresh->attr('content')}'\n");
			...
			// XXX not secure ?!
			haoxinqing_me_cj::ajaxAllowURL($urlRefresh);
//			$urlRefresh = urldecode($urlRefresh);
			...
			if ($xhr->getLastResponse()->isSuccessful()) {
				...
				return call_user_func_array(
					array('haoxinqing_me_cjPlugin_WebBrowser', 'browserReceive'), array($xhr)
				);
			}
		} else
			return $pq;
	...
	public static function hadleClick($e, $callback = null) {
		$node = haoxinqing_me_cj::pq($e->target);
		$type = null;
		if ($node->is('a[href]')) {
...
//			'success' => $e->data[1],
		...
		$xhr = haoxinqing_me_cj::ajax($options, $xhr);
		if ((! $callback || !($callback instanceof Callback)) && $e->data[1])
			$callback = $e->data[1];
		if ($xhr->getLastResponse()->isSuccessful() && $callback)
			haoxinqing_me_cj::callbackRun($callback, array(
